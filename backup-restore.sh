...
ENV_FILE=".env"
SCRIPT_REPO_URL="https://raw.githubusercontent.com/Masouuuuuuud/remnawave-backup-masouud/refs/heads/main/backup-restore.sh"
...
main_menu() {
    while true; do
        ...
        echo ""
        echo "   1. Creating a backup manually"
        echo "   2. Restoring from backup"
        echo ""
        echo "   3. Setting up automatic sending and notifications"
        echo "   4. Setting up the sending method"
        echo "   5. Changing the script configuration"
        echo ""
        echo "   6. Script update"
        echo "   7. Deleting a script"
        echo ""
        echo "   0. Exit"
        ...
        case $choice in
            ...
            7) remove_script ;;
            0) echo "Выход..."; exit 0 ;;
            *) print_message "ERROR" "Invalid input. Please select one of the suggested options.." ; read -rp "Press Enter to continue..." ;;
        esac
    done
}
...
if ! command -v jq &> /dev/null; then
    ...
    if command -v apt-get &> /dev/null; then
        ...
        print_message "ERROR" "Unable to find package manager apt-get. Install 'jq' manually."
        exit 1
    fi
fi
